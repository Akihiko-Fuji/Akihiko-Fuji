...
情報システム部門やソフトウェア開発会社で働いた経験はなく、もともとはIT畑の人間ではありません。仕事の出発点は、製造現場と生産技術です。

生産設備の導入、工程設計、物流、製造工程の改善などに関わる中で、設備や作業方法を変えるだけでは解決できない問題に、たびたび直面してきました。

業務フローや情報の流れまで見直す必要があり、そのために必要なシステムを自分で設計し、プログラムを書くようになりました。そこから少しずつ、ハードウェアの評価、導入、運用支援まで扱う範囲を広げてきました。

現在は、製造技術、業務再設計、システム構築に取り組んでいます。
製造現場にある複雑な業務を整理し、実際に運用できる仕組みに落とし込み、継続して使えるシステムとして実装することが主な仕事です。

アプリやツールを作ること自体が目的ではありません。
業務フロー、情報の流れ、運用ルール、設備、人の動きまで含めて見直していくため、結果として **BPR（Business Process Re-engineering）** に近い仕事になることもよくあります。

理屈としては正しくても現場では使いにくいもの、資料上はきれいでも実装まで届かないもの、試作だけで終わってしまうソフトウェアではなく、実際の現場で使われ、動き続ける仕組みを作ることに関心があります。
...
様々な撮像フォーマットで撮影をするため、焦点距離に対する画角と許容錯乱円が同等なF値を算出するプログラムを作成しました。
